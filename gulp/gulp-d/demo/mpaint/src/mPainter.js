;(function (window, undefined) {
     ...
     * Initialization
     */
    mPainter = function (toolhook, hook, options) {
        var defaults = {
                id: 'm-board',
                cursor_id: 'm-cursor',
            ...
            config = {
                ...
                WIDGETS: {
                    ...
                    TOOL: {
                        TYPE: ['FREE', 'LINE', 'RECT', 'ELLIPSE', 'RIGHT', 'FALSE'],
                        FREE: {
                            NAME: 'FREE',
                            ICON: '\u270E'
                        },
    ...
    mPainter.prototype = {
        ...
        getSvg: function () {
            var _svg = document.getElementById(this.options.id)
    ...
    function renderWidgets(widget_conf) {
        var toolhook = _get(_self.options.toolhook_id),
        ...
        toolhook.insertBefore(button_groups, toolhook.childNodes[0]);
    }
    ...
    function draw(config, points) {
        ...
        case _self.options.WIDGETS.TOOL.LINE.NAME:
            ...
            break;
        case _self.options.WIDGETS.TOOL.FALSE.NAME:
            drawFalse(config, points);
            break;
        case _self.options.WIDGETS.TOOL.RIGHT.NAME:
            drawRight(config, points);
            break;
    ...
    function drawRight(config, points) {
        var element_id = _self.options.element_prefix + config.element_index,
            element = _get(element_id),
    ...
    function getPointFromEvent(event) {
        var e = event || window.event,
            x, y;
        var goal = _get(_self.options.hook_id);
        if (!isTouch()) {
            x = e.offsetX;
            y = e.offsetY;
        } else if (e.targetTouches.length) {
            x = e.targetTouches[0].clientX;
            y = e.targetTouches[0].clientY - goal.offsetTop + window.scrollY;
            console.log(x,y)
        } else {
            x = e.changedTouches[0].clientX;
            y = e.changedTouches[0].clientY - goal.offsetTop + window.scrollY;
        }
